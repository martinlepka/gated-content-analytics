    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-[10px] text-gray-500 font-cyber tracking-wider">Loading content...</span>
        </div>
      </div>
    )
  }
      ...
      <header className="cyber-header sticky top-0 z-40 px-4 py-2">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] text-gray-500 tracking-wider">CONTENT METRICS</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="cyber-stat p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[9px] text-gray-500 tracking-wider">ASSETS</span>
            </div>
            <div className="font-cyber text-2xl text-neon-cyan">{content.length}</div>
          </div>
          <div className="cyber-stat p-4">
            <div className="flex items-center gap-2 mb-1">
              <TrendingUp className="h-4 w-4 text-neon-green" />
              <span className="text-[9px] text-gray-500 tracking-wider">DOWNLOADS</span>
            </div>
            <div className="font-cyber text-2xl text-neon-green">{totalDownloads}</div>
          </div>
          <div className="cyber-stat p-4">
            <div className="flex items-center gap-2 mb-1">
              <Users className="h-4 w-4 text-neon-purple" />
              <span className="text-[9px] text-gray-500 tracking-wider">AVG QUALITY</span>
            </div>
            <div className="font-cyber text-2xl text-neon-purple">{avgQuality}%</div>
          </div>
        </div>
        ...
        <div className="cyber-table overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {content.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-3 py-8 text-center text-gray-400 font-cyber text-xs">
                    No content data
                  </td>
                </tr>
              ) : (
                content.map((item, index) => (
                  <tr key={index} className="cyber-row">
                    <td>
                      <span className="text-[12px] text-gray-800 truncate block max-w-[200px]" title={item.content_name}>
                        {item.content_name}
                      </span>
                    </td>
                    ...
                    <td className="text-center">
                      <span className="text-[12px] text-gray-600 font-mono">{item.avg_score}</span>
                    </td>
                    <td className="text-center">
